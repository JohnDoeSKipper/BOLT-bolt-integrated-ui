...
Used when the client knows they have solar but can't remember the capacity.
"""
...
SOLAR_PEAK_CURVE_FRACTION = 0.50  # at midday, output ≈ 50% of nameplate kWp (Malaysia avg)


def detect_has_solar(df: pd.DataFrame) -> tuple[bool, str]:
    """Returns (has_solar, reason)."""
...
def estimate_solar_capacity_kwp(df: pd.DataFrame) -> dict:
    """Estimate installed solar capacity in kWp."""
    d = df.copy()
    d["hour"] = d["timestamp"].dt.hour
