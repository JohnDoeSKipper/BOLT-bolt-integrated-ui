...
class SimulationState:
    ...
    # How many most-recent verified rows per horizon feed the bias estimate.
    # Larger = more stable but slower to track regime shifts. Set per-site
    # via initialize_simulation(... bias_window=...). E (industrial, stable
    # drift) benefits from 18-24; SoL (volatile solar) wants 8-10.
    bias_window: int = 12
...
def initialize_simulation(
    ...
    bias_window controls how many recent verified rows per horizon feed the
    per-horizon bias correction. Sites with stable systematic drift (E)
    benefit from wider windows (18-24). Volatile sites (SoL) want tighter.
    """
...
def _smooth3(arr: np.ndarray) -> np.ndarray:
    """3-tap [0.25, 0.5, 0.25] smoothing with reflective edges.

    Standard np.convolve(..., mode='same') zero-pads, which halves the
    smoothed value at the array boundaries — a real bug for per-horizon
    bias correction, where the h=48 entry is the most operationally
    important (and the only one where solar/long-horizon drift accumulates
    over a full day). Reflective padding preserves edge magnitudes.
    """
    ...
    # Reflect at the boundaries so the kernel still sums to 1
    out[0]  = 0.5 * arr[0] + 0.5 * arr[1]
    out[-1] = 0.5 * arr[-1] + 0.5 * arr[-2]
    return out
...
def _compute_calibration(
    state: SimulationState,
    output_steps: int,
    bias_window: int = 12,           # ↑ from 6 — baseline showed E drifting
                                     # to -71 kW @ h=48; small window was
                                     # too noisy to track the long-horizon gap.
    ...
    # Light [0.25, 0.5, 0.25] smoothing so the per-horizon profile doesn't
    # introduce its own kinks. _smooth3 uses edge reflection instead of
    # numpy's zero-padded convolve — the old code halved the bias correction
    # at h=0 and h=output_steps-1 (e.g. recovered only ~50% of the -71 kW
    # E-site h=48 drift seen in baseline backtests).
    if output_steps >= 3:
        bias_profile = _smooth3(bias_profile)

    # Per-horizon conformal half-width from in-sim verified residuals.
    # (We tried a training-residual fallback for sparse horizons but the
    # A/B showed it was net noise — 400-tick runs accumulate enough in-sim
    # samples at every horizon to make the fallback rarely fire, and when
    # it did fire the validation-set quantile was systematically narrower
    # than live distribution, slightly over-confidencing the bands.)
    for h in range(1, output_steps + 1):
...
def advance_one_tick(
    ...
    bias_profile, conformal_hw = _compute_calibration(
        state, output_steps=48, bias_window=state.bias_window,
    )
    ...
    # Fix #3: mirror the calibration onto the forecaster so a fresh
    # reload (e.g. next Predictor open) retains it as the default for
    # standalone forecast() calls. Persisted via save/load.
    forecaster.bias_profile = bias_profile
    forecaster.conformal_half_width = conformal_hw
